...
#End Region
    ...
    Public Sub checkIfApproved() 'Sjekker at det har gått minst tre måneder siden sist tapping
        ...
        Try
            ...
            If UserPage.txtQuarantine.Text = "Livstid" Then
                MsgBox("Du har karantene på livstid og kan ikke ta spørreskjema på nytt.", MsgBoxStyle.Critical, "Karantene")
            Else
                If UserPage.txtDrain.Text = "Ikke tappet" Then
                    UserPage.Hide()
                    Me.Show()
                ElseIf lastDrain >= threeMounthAgo Then 'Hvis "siste tapping" er eldre eller lik 90 dager bak i tid så kommer denne msgboxen opp
    ...
    Private Sub SaveAnswers()
        ...
        Try
            ...
            Else
                ...
                comm = New MySqlCommand(query, connection)
                reader = comm.ExecuteReader
                MsgBox("Du har fått en karantene på: " & totalQT, MsgBoxStyle.Information, "Spørreskjema")
            End If
